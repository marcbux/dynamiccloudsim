...
public class Task extends HeterogeneousCloudlet implements Comparable<Task> {
	...
	public Task(final String name, final String params, final Workflow workflow, final int userId, final int cloudletId, final long miLength,
	    final long ioLength, final long bwLength, final int pesNumber, final long cloudletFileSize, final long cloudletOutputSize,
	    final UtilizationModel utilizationModelCpu, final UtilizationModel utilizationModelRam, final UtilizationModel utilizationModelBw) {
		super(cloudletId, miLength, ioLength, bwLength, pesNumber, cloudletFileSize, cloudletOutputSize, utilizationModelCpu, utilizationModelRam,
		    utilizationModelBw);
		this.name = name;
		this.params = params;
	...
	public Task(Task task) {
		this(task.getName(), task.getParams(), task.getWorkflow(), task.getUserId(), task.getCloudletId(), task.getMi(), task.getIo(), task.getBw(), task
		    .getNumberOfPes(), task.getCloudletFileSize(), task.getCloudletOutputSize(), task.getUtilizationModelCpu(), task.getUtilizationModelRam(), task
