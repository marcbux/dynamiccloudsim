...
public class Workflow {
	...
	public Workflow() {
		workflow = new DirectedSparseMultigraph<>();
	}
	...
	private void computeWidthArray() {
		setBreadth(new int[Task.getMaxDepth() + 1]);
	...
	public List<Task> getSortedChildren(Task task) {
		List<Task> children = new ArrayList<>(getChildren(task));
	...
	public void visualize(int width, int height) {
		...
		StaticLayout<Task, DataDependency> sl = new StaticLayout<>(workflow, new TaskPositionInWorkflowTransformer(width, height, getBreadth()));
		sl.setSize(size);
		VisualizationViewer<Task, DataDependency> vv = new VisualizationViewer<>(sl);
		...
		Transformer<Task, Paint> vertexColor = new Transformer<Task, Paint>() {
			...
			public Paint transform(Task task) {
				...
				case 2:
					return new Color(31, 73, 125); // index
				case 3:
					return new Color(228, 108, 10); // align
				case 4:
					return new Color(119, 147, 60); // samtools view
				case 5:
					return new Color(244, 238, 19); // samtools sort
				case 6:
					return new Color(96, 74, 123); // samtools merge
				case 7:
					return new Color(96, 74, 123); // samtools merge
				case 8:
					return new Color(149, 55, 53); // pileup
				case 9:
					return new Color(49, 133, 156); // varscan
				case 10:
					return new Color(20, 137, 89); // diff
					// case 6: return new Color(96,74,123);
					// case 7: return new Color(96,74,123);
				default:
					return new Color(127, 127, 127); // stage in / out
				}
			}
		};
		Transformer<Task, Shape> vertexSize = new Transformer<Task, Shape>() {
			...
			public Shape transform(Task task) {
				...
				if (getBreadth()[task.getDepth()] > 25) {
					circle = new Ellipse2D.Double(-5, -15, 10, 30);
				}
				if (getBreadth()[task.getDepth()] > 250) {
		...
		// to file
		VisualizationImageServer<Task, DataDependency> vis = new VisualizationImageServer<>(vv.getGraphLayout(), vv.getGraphLayout().getSize());
		...
		BufferedImage image = (BufferedImage) vis.getImage(new Point2D.Double(vv.getGraphLayout().getSize().getWidth() / 2, vv.getGraphLayout().getSize()
		    .getHeight() / 2), new Dimension(vv.getGraphLayout().getSize()));
		try {
